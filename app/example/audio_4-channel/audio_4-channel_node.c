...
#elif defined(NODE3)
#include "swc_cfg_node3.h"
#elif defined(NODE4)
#include "swc_cfg_node4.h"
#endif

/* CONSTANTS ******************************************************************/
#define SAC_MEM_POOL_SIZE      4800
#define SAC_PAYLOAD_SIZE       120
#define SAC_LATENCY_QUEUE_SIZE 10
#define SWC_MEM_POOL_SIZE      4800
...
int main(void)
{
    ...
    while (1) {
        iface_volume_handling(volume_up, volume_down);
...
static void app_swc_core_init(swc_error_t *err)
{
    ...
    rx_from_coord_conn = swc_connection_init(node, rx_conn_cfg, &swc_hal, err);
    if (*err != SWC_ERR_NONE) {
        return;
    }
    ...
    for (uint8_t i = 0; i < ARRAY_SIZE(channel_sequence); i++) {
        ...
        swc_connection_add_channel(rx_from_coord_conn, node, rx_channel_cfg, err);
        if (*err != SWC_ERR_NONE) {
            return;
        }
    }
...
static void app_audio_core_init(sac_error_t *sac_err)
{
    ...
    iface_audio_dma_complete_callbacks(audio_i2s_tx_complete_cb, NULL);

    app_audio_core_volume_interface_init(&volume_iface);
    app_audio_core_cdc_interface_init(&cdc_iface);

    swc_producer_instance.connection = rx_from_coord_conn;
     ...
     * ==============
     *
     * Input:      Mono stream of 61 samples @ 48 kHz/16 bits is received over the air from the Coordinator.
     * Processing: Digital volume control followed by clock drift compensation.
     * Output:     Mono stream of 61 samples @ 48 kHz/16 bits is played using the MAX98091 hardware audio codec.
     *
     * +-----+    +----------------+    +-----+    +-------+
...
static void print_stats(void)
{
    ...
    string_length += swc_connection_format_stats(rx_from_coord_conn, node, stats_string + string_length, sizeof(stats_string) - string_length);
