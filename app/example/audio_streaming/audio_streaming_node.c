...
static src_cmsis_instance_t downsampling_instance;
static sac_processing_t *downsampling_processing;
...
static void app_swc_core_init(swc_error_t *err)
{
    ...
    swc_node_add_radio(node, radio2_cfg, &swc_hal, err);
    if (*err != SWC_ERR_NONE) {
        return;
    }
#endif
    ...
    swc_connection_cfg_t tx_conn_cfg = {
        ...
        .max_payload_size = SAC_BACK_CHANNEL_PAYLOAD_SIZE + sizeof(sac_header_t),
...
static void app_audio_core_init(sac_error_t *sac_err)
{
    ...
    sac_processing_interface_t downsampling_iface;
    ...
    primary_sac_pipeline = sac_pipeline_init("SWC -> Codec", swc_producer,
                             primary_pipeline_cfg, max98091_consumer, sac_err);
    if (*sac_err != SAC_ERR_NONE) {
        return;
    }

    sac_pipeline_add_processing(primary_sac_pipeline, volume_processing, sac_err);
    sac_pipeline_add_processing(primary_sac_pipeline, cdc_processing, sac_err);
     ...
     * ========================
     *
     * Input:      Mono stream of 90 samples @ 48 kHz/16 bits is recorded using the MAX98091 hardware audio codec.
     * Processing: Downsampling from 48 kHz to 16 kHz.
     * Output:     Mono stream of 30 samples @ 16 kHz/16 bits is sent over the air to the Coordinator.
     *
     * +-------+    +-----------------+    +-----+
     * | Codec | -> | Downsampling 3x | -> | SWC |
     * +-------+    +-----------------+    +-----+
     */
    ...
    downsampling_instance.cfg.divide_ratio = SAC_SRC_THREE;
    downsampling_instance.cfg.multiply_ratio = SAC_SRC_ONE;
    downsampling_instance.cfg.payload_size = max98091_producer_cfg.audio_payload_size;
    downsampling_instance.cfg.bit_depth = max98091_producer_cfg.bit_depth;
    downsampling_processing = sac_processing_stage_init((void *)&downsampling_instance,
                                                        "Audio Downsampling",
                                                        downsampling_iface,
                                                        sac_err);
    if (*sac_err != SAC_ERR_NONE) {
    ...
    sac_endpoint_cfg_t swc_consumer_cfg = {
        ...
        .channel_count      = 1,
        .bit_depth          = SAC_16BITS,
        .audio_payload_size = SAC_BACK_CHANNEL_PAYLOAD_SIZE,
    ...
    secondary_sac_pipeline = sac_pipeline_init("Codec -> SWC", max98091_producer,
                                                 secondary_pipeline_cfg, swc_consumer, sac_err);
    if (*sac_err != SAC_ERR_NONE) {
        return;
    }

    sac_pipeline_add_processing(secondary_sac_pipeline, downsampling_processing, sac_err);
...
static void volume_up(void)
{
    ...
    sac_processing_ctrl(volume_processing, primary_sac_pipeline, SAC_VOLUME_INCREASE, SAC_NO_ARG, &err);
}
...
static void volume_down(void)
{
    ...
    sac_processing_ctrl(volume_processing, primary_sac_pipeline, SAC_VOLUME_DECREASE, SAC_NO_ARG, &err);
}
