...
static sac_processing_t *user_data_rx_processing;
...
static uint16_t app_audio_user_data_process_rx(void *instance, sac_pipeline_t *pipeline, sac_header_t *header, uint8_t *data_in,
...
static void app_audio_core_init(sac_error_t *sac_err)
{
    ...
    sac_processing_interface_t user_data_rx_iface;
    ...
    app_audio_core_user_data_interface_init(&user_data_rx_iface);
     ...
     * Processing: A byte of trailing data is extracted from the audio packet followed by digital volume control
     *             followed by clock drift compensation.
     * Output:     Stereo stream of 34 samples @ 48 kHz/16 bits is played using the MAX98091 hardware audio codec.
     *
     * +-----+    +------------------------+    +----------------+    +-----+    +-------+
     * | SWC | -> | Extract User Data Byte | -> | Digital Volume | -> | CDC | -> | Codec |
     * +-----+    +------------------------+    +----------------+    +-----+    +-------+
     */
    ...
    user_data_rx_processing = sac_processing_stage_init(NULL, "User Data RX", user_data_rx_iface, sac_err);
    if (*sac_err != SAC_ERR_NONE) {
        return;
    }
    ...
    sac_pipeline_add_processing(sac_pipeline, user_data_rx_processing, sac_err);
...
static void app_audio_core_user_data_interface_init(sac_processing_interface_t *iface)
{
    iface->init = NULL;
    iface->ctrl = NULL;
    iface->process = app_audio_user_data_process_rx;
    iface->gate = NULL;
}
                                               ...
                                               uint16_t size, uint8_t *data_out, sac_error_t *err)
{
    ...
    *err = SAC_ERR_NONE;
    ...
    if (size > SAC_PAYLOAD_SIZE) {
        iface_payload_received_status(data_in[(size - 1)]);
        output_size = size - 1;
