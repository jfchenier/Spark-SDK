/** @file  pairing_gaming_hub_keyboard.c
 *  @brief The keyboard device is part of the gaming hub to demonstrate a point to multipoint pairing application.
 *
...
#include "sac_api.h"
#include "swc_api.h"
#include "swc_cfg_keyboard.h"
#include "swc_pairing.h"
#include "swc_utils.h"

/* CONSTANTS ******************************************************************/
#define SWC_MEM_POOL_SIZE          8000
#define PAIRING_DEVICE_ROLE        3
...
static swc_pairing_t app_pairing = {0};
...
static void send_keyboard_pressed_state(void);
static void send_keyboard_released_state(void);
...
int main(void)
{
    ...
    while (1) {
        ...
        if (!is_device_paired) {
            /* Since the device is unpaired the send_keyboard_pressed_state should do nothing */
            iface_button_handling_osr(NULL, enter_pairing_mode);
        } else {
            iface_button_handling_osr(send_keyboard_pressed_state, unpair_device);
            iface_button_handling_osf(send_keyboard_released_state, NULL);
        }
    }
}
...
static void app_swc_core_init(swc_pairing_t *app_pairing, swc_error_t *err)
{
    ...
    /* ** Keyboard receiving Beacon from Dongle connection ** */
    ...
    /* ** Keyboard sending to Dongle connection ** */
...
/** @brief Send the state of the button on a button press event.
 */
static void send_keyboard_pressed_state(void)
{
    swc_error_t swc_err = SWC_ERR_NONE;
    uint8_t *keyboard_press_status = NULL;

    swc_connection_get_payload_buffer(tx_to_dongle_conn, &keyboard_press_status, &swc_err);
    if (keyboard_press_status != NULL) {
        *keyboard_press_status = 0x01;
        swc_connection_send(tx_to_dongle_conn, keyboard_press_status, KEYBOARD_PAYLOAD_SIZE, &swc_err);
    }
