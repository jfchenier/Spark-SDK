...
/* PUBLIC FUNCTIONS ***********************************************************/
sac_mixer_module_t *sac_mixer_module_init(sac_mixer_module_cfg_t cfg, mem_pool_t *mem_pool, sac_error_t *sac_error)
{
    *sac_error = SAC_ERR_NONE;
    ...
    if (sac_mixer_module == NULL) {
        *sac_error = SAC_ERR_NOT_ENOUGH_MEMORY;
        return NULL;
    }
    ...
    if (cfg.nb_of_inputs < MIN_NB_OF_INPUTS ||
        ...
        *sac_error = SAC_ERR_MIXER_INIT_FAILURE;
        return NULL;
    }
