...
/* PUBLIC FUNCTIONS ***********************************************************/
void sac_volume_init(void *instance, sac_pipeline_t *pipeline, mem_pool_t *mem_pool, sac_error_t *err)
{
    (void)pipeline;
    (void)mem_pool;
...
uint32_t sac_volume_ctrl(void *instance, sac_pipeline_t *pipeline, uint8_t cmd, uint32_t arg, sac_error_t *err)
{
    (void)pipeline;
    (void)arg;
...
uint16_t sac_volume_process(void *instance, sac_pipeline_t *pipeline, sac_header_t *header, uint8_t *data_in, uint16_t size, uint8_t *data_out, sac_error_t *err)
{
    (void)pipeline;
    (void)header;
    ...
    *err = SAC_ERR_NONE;
    ...
    if ((vol_inst->_internal.volume_threshold != SAC_VOLUME_MAX) || (vol_inst->_internal.volume_factor != SAC_VOLUME_MAX)) {
        ...
        switch (vol_inst->bit_depth) {
        case SAC_16BITS:
            apply_volume_factor_16bits((int16_t *)data_in, (size / SAC_16BITS_BYTE), (int16_t *)data_out, vol_inst->_internal.volume_factor);
            break;
        case SAC_20BITS:
            apply_volume_factor_32bits((int32_t *)data_in, (size / SAC_20BITS_BYTE), (int32_t *)data_out, vol_inst->_internal.volume_factor);
            break;
        case SAC_24BITS:
            apply_volume_factor_32bits((int32_t *)data_in, (size / SAC_24BITS_BYTE), (int32_t *)data_out, vol_inst->_internal.volume_factor);
            break;
        case SAC_32BITS:
            apply_volume_factor_32bits((int32_t *)data_in, (size / SAC_32BITS_BYTE), (int32_t *)data_out, vol_inst->_internal.volume_factor);
            break;
        default:
            return 0;
        }
