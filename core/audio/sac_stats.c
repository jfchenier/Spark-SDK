...
int sac_pipeline_format_stats(sac_pipeline_t *pipeline, char *buffer, uint16_t size)
{
    ...
    string_length = snprintf(buffer, size,
                             ...
                             pipeline->name,
                             producer_str,
                             buffer_load_str, pipeline->_statistics.producer_buffer_load,
                             buffer_size_str, pipeline->_statistics.producer_buffer_size,
                             producer_packets_corrupted_count_str, pipeline->_statistics.producer_packets_corrupted_count,
                             consumer_str,
                             buffer_load_str, pipeline->_statistics.consumer_buffer_load,
                             buffer_size_str, pipeline->_statistics.consumer_buffer_size,
                             producer_buffer_overflow_count_str, pipeline->_statistics.producer_buffer_overflow_count,
                             consumer_buffer_overflow_count_str, pipeline->_statistics.consumer_buffer_overflow_count,
                             consumer_buffer_underflow_count_str, pipeline->_statistics.consumer_buffer_underflow_count,
                             cdc_inflated_packets_count_str, pipeline->_statistics.cdc_inflated_packets_count,
                             cdc_deflated_packets_count_str, pipeline->_statistics.cdc_deflated_packets_count);
...
{
    return pipeline->_statistics.cdc_inflated_packets_count;
}

uint32_t sac_pipeline_get_cdc_deflated_packets_count(sac_pipeline_t *pipeline)
{
    return pipeline->_statistics.cdc_deflated_packets_count;
}
...
{
    return pipeline->_statistics.consumer_link_margin_min_peak;
}

uint32_t sac_pipeline_get_consumer_cca_peak(sac_pipeline_t *pipeline)
{
    return pipeline->_statistics.consumer_cca_fail_count_peak;
}
...
void sac_pipeline_reset_stats(sac_pipeline_t *pipeline)
{
    ...
    pipeline->_statistics.consumer_link_margin_min_peak = UINT32_MAX;
}
