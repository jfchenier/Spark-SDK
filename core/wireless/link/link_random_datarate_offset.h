...
typedef struct link_rdo {
    uint16_t offset;                     /*!< Current offset value, in PLL cycle. */
    uint16_t rollover_n;                 /*!< Offset rollover value. */
    uint16_t increment_step;             /*!< Number of timeslot between each offset increment. */
    uint8_t offset_u8[sizeof(uint16_t)]; /*!< Offset value split for uint8_t transfer. */
    bool enabled;                        /*!< RDO enable flag. */
} link_rdo_t;
