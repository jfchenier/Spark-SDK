...
typedef enum fragment_transfer_type {
    FULL_FRAME_TRANSFER_TYPE        = 0b000, /*!< One fragment upper layer frame */
    NON_LAST_FRAGMENT_TRANSFER_TYPE = 0b010, /*!< First or middle fragment */
    LAST_FRAGMENT_TRANSFER_TYPE     = 0b100, /*!< Last fragment */
    ABORT_TRANSFER_TYPE             = 0b110, /*!< Abort messsage */
...
typedef struct transaction_control {
    uint8_t transfer_type : 3;  /*!< Frame fragment transfer type */
    uint8_t transaction_id : 5; /*!< Transaction unique ID */
} __packed transaction_control_t;

/** @brief WPS fragment full frame fragment structure.
 */
typedef struct full_frame {
    transaction_control_t transaction_control; /*!< Transaction control field */
...
typedef struct first_fragment {
    transaction_control_t transaction_control; /*!< Transaction control field */
    uint8_t fragment_number;                   /*!< Fragment index */
    uint16_t total_upper_layer_frame_size;     /*!< Total upper layer frame size */
...
static inline void send_full_frame(wps_connection_t *connection, uint8_t transaction_id, uint8_t *payload, size_t size,
                                   wps_error_t *err);
static inline size_t send_first_fragment(wps_connection_t *connection, uint8_t transaction_id, uint8_t **payload,
                                         size_t size, wps_error_t *err);
static inline size_t send_middle_fragment(wps_connection_t *connection, uint8_t transaction_id, uint8_t **payload,
                                          size_t size, uint8_t fragment_number, wps_error_t *err);
static inline void send_last_fragment(wps_connection_t *connection, uint8_t transaction_id, uint8_t **payload,
                                      size_t size, uint8_t fragment_number, wps_error_t *err);
...
static void wps_read_error_flush(wps_connection_t *connection, wps_error_t *err);
...
void wps_frag_init(wps_connection_t *connection, void *meta_tx_buffer, uint32_t meta_tx_size)
{
    ...
    circular_queue_init(&connection->frag.meta_data_queue_tx, meta_tx_buffer, sizeof(uint16_t), meta_tx_size);
...
void wps_frag_send(wps_connection_t *connection, uint8_t *payload, size_t size, wps_error_t *err)
{
                                   ...
                                   wps_error_t *err)
{
    ...
    wps_get_free_slot(connection, &payload_in, err);
    if (*err != WPS_NO_ERROR) {
        return;
    }
