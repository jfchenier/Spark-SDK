                                ...
                                uint16_t data_size, const char *xlayer_queue_name)
{
    ...
    num_nodes += XLAYER_QUEUE_EXTRA_NODES;
    ...
    for (uint8_t i = 0; i < num_nodes; i++) {
        ...
        node_ptr->xlayer.frame.header_memory_size = HEADER_MAX_SIZE;
        node_ptr->xlayer.frame.header_memory = &data[HEADER_MAX_SIZE - 1];
        node_ptr->xlayer.frame.header_begin_it = node_ptr->xlayer.frame.header_memory;
        node_ptr->xlayer.frame.header_end_it = node_ptr->xlayer.frame.header_memory;
        node_ptr->xlayer.frame.payload_memory_size = data_size;
        node_ptr->xlayer.frame.payload_memory = &data[HEADER_MAX_SIZE];
        node_ptr->xlayer.frame.payload_begin_it = node_ptr->xlayer.frame.payload_memory;
        node_ptr->xlayer.frame.payload_end_it = node_ptr->xlayer.frame.payload_memory;
        node_ptr++;
        data += (data_size + HEADER_MAX_SIZE);
    }
    ...
    num_bytes_used =  (num_nodes * ((sizeof(xlayer_queue_node_t) + data_size + HEADER_MAX_SIZE)));
